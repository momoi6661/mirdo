...
```json
{
  ...
  "action_line": [{
    ...
    "command_payload": {"target_nav_point": "food_cabinet", "marker_role": "approach"},
    "reason": "先到柜前确认库存",
...
- `action_line`：0 到 4 个有前因后果的步骤，只有首个 pending 步骤交给 Godot。
- `current_step_id`：本回合唯一执行步骤的 `step_id`。
- `action_line[].command`：使用 `look_at_player|go_to_nav_point|go_to_object|follow_player|stop_follow|pick_up_item|take_from_container|use_item|eat_item|give_item_to_player`。
- `action_line[].command_payload`：目标字段必须来自当前 perception 或 `context.ai_nav_points[].id`。
- `action_line[].reason/expected_result`：说明为什么做、完成后应该观察什么。
- `take_from_container` 必须等待 Godot 回报库存减少；`give_item_to_player` 必须等待玩家接受、拒绝或超时事件，不能在同一回合假设后续已经发生。
...
- 饿/渴：`sorrow` + `small_nod`；如果要喝水，水源只来自食品柜/备用食品柜内的库存，或地上可见的水实体。
...
> 当前 Server 与 Mirdo Godot 组件只使用 `action_line`。本文较早章节中出现的顶层 `command` 示例均为历史记录，不属于当前接口；实现和调试请以 `action_line`、`current_step_id`、`action_step`、`action_result` 为准。
