...
func _run() -> void:
	...
	await _test_router_does_not_replay_line_after_world_was_seen()
...
func _test_router_does_not_replay_line_after_world_was_seen() -> void:
	var router_script: Script = load("res://scripts/character_ai/components/character_subtitle_router_component.gd") as Script
	if router_script == null:
		return
	var root_node := Node3D.new()
	...
	_expect(overlay.show_calls == 0, "line already seen on world subtitle should not replay as 2D overlay")
	root_node.queue_free()
	await process_frame
