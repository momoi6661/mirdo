...
func _init() -> void:
	...
	_expect(not bool(root.get_node("WeaponEquipmentCabinet/ContainerPanel3D").get("allow_item_dragging")), "武器柜面板暂不允许拖动物品")
	_expect(not bool(root.get_node("MedicalCabinet/ContainerPanel3D").get("allow_item_dragging")), "医疗柜面板暂不允许拖动物品")
