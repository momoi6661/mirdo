...
const DEFAULT_RULE_SET_PATH: String = "res://resources/xiaokong/status/xiaokong_default_status_rule_set.tres"
...
func _resolve_target_root(payload: Dictionary) -> Node3D:
	...
	if payload.is_empty():
		return owner_root
	var path_text := String(payload.get("xiaokong_path", "")).strip_edges()
