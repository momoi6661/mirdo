...
const OUT_PATH := "res://resources/animate/Kimodo/mirdo_walk_forward_loop_smooth.res"
const CUT_START := 2.23333333333333
const CUT_END := 4.7
const EPS := 0.00001
...
func key_range(anim: Animation, track: int, t: float) -> Array:
	...
	var f := 0.0 if abs(t1 - t0) < EPS else (t - t0) / (t1 - t0)
...
func _init():
	...
	for i in range(src.get_track_count()):
		...
		for k in range(src.track_get_key_count(i)):
			...
			insert_key(out, dst, kt - CUT_START, val, src.track_get_key_transition(i, k))
		# 为了接缝丝滑，非 Root 的最后一帧强制回到首帧姿态；Root 位置保留位移，供 root motion 使用。
		if not (typ == Animation.TYPE_POSITION_3D and name == "Root"):
			v1 = v0
		insert_key(out, dst, dur, v1)
