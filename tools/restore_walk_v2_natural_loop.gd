...
const BACKUP_PATH := "res://resources/animate/Kimodo/walk_forward_loop_v2_before_restore_natural.res"
# Previous smooth self-loop candidate.
const CUT_START := 2.23333333333333
const CUT_END := 3.83333333333333
const EPS := 0.00001
...
func _init():
	...
	for i in range(src.get_track_count()):
		...
		# Keep natural sampled boundary keys. Do NOT force last pose to equal first pose.
	...
	print("SAVE err=", err, " path=", OUT_PATH, " natural_cut=", CUT_START, "..", CUT_END, " length=", dur)
