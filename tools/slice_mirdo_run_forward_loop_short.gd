...
const OUT_PATH := "res://resources/animate/Kimodo/mirdo_run_forward_loop_short.res"
const CUT_START := 2.03333333333333
const CUT_END := 2.86666666666667
const EPS := 0.00001
...
func _init():
	...
	for i in range(src.get_track_count()):
		...
		# 非 Root 轨道末帧闭合到首帧；Root 保留前进位移用于 root motion。
	...
	print("SAVE err=", err, " out=", OUT_PATH, " length=", out.length, " loop=", out.loop_mode, " tracks=", out.get_track_count())
