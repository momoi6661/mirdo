...
# This start was the best match to mirdo_stand_to_walk_fast.res end.
const CUT_START := 2.8
...
func _init():
	...
	for i in range(src.get_track_count()):
		...
		# Loop closure: copy first pose to last frame, but keep Root forward displacement.
	...
	print("SAVE err=", err, " path=", OUT_PATH, " cut=", CUT_START, "..", CUT_END, " length=", dur, " loop=", out.loop_mode)
	quit(err)
