...
const ANIM_PATH := "res://resources/animate/Kimodo/seated_sleepy.res"
const FPS := 30.0
var important := {"Hips":3.0,"Spine":1.5,"Chest":1.5,"UpperChest":1.2,"Head":1.2,"LeftUpperLeg":2.0,"RightUpperLeg":2.0,"LeftUpperArm":1.0,"RightUpperArm":1.0}
...
func _init():
	...
	print("time rootZ hipsY poseDelta speedZ")
	for f in range(0,int(ceil(anim.length*FPS))+1):
		...
		var r:=pos(anim,root_t,t)
		var h:=pos(anim,hips_t,t)
		var pd:float=0.0 if t<=0.0 else pose_delta(anim,a,t)
		var sp:float=0.0 if t<=0.0 else (r.z-pos(anim,root_t,a).z)/max(t-a,0.0001)
		print("t=","%0.3f"%t," rootZ=","%0.4f"%r.z," hipsY=","%0.4f"%h.y," pose=","%0.3f"%pd," speedZ=","%0.3f"%sp)
	quit()
