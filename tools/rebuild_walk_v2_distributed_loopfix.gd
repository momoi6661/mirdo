extends SceneTree

const SRC_PATH := "res://resources/animate/Kimodo/walking_into_running.res"
const OUT_PATH := "res://resources/animate/Kimodo/walk_forward_loop_v2.res"
const BACKUP_PATH := "res://resources/animate/Kimodo/walk_forward_loop_v2_before_distributed_loopfix.res"
const CUT_START := 2.8
const CUT_END := 4.36666666666667
const EPS := 0.00001

func track_name(anim: Animation, i: int) -> String:
	var p := str(anim.track_get_path(i))
	var idx := p.rfind(":")
	return p.substr(idx + 1) if idx >= 0 else p

func key_range(anim: Animation, track: int, t: float) -> Array:
	var n := anim.track_get_key_count(track)
	if n <= 0: return [0, 0, 0.0]
	if t <= anim.track_get_key_time(track, 0): return [0, 0, 0.0]
	if t >= anim.track_get_key_time(track, n - 1): return [n - 1, n - 1, 0.0]
	var lo := 0
	var hi := n - 1
	while hi - lo > 1:
		var mid := int((lo + hi) / 2)
		if anim.track_get_key_time(track, mid) <= t:
			lo = mid
		else:
			hi = mid
	var t0 := anim.track_get_key_time(track, lo)
	var t1 := anim.track_get_key_time(track, hi)
	var a: float = 0.0 if abs(t1 - t0) < EPS else (t - t0) / (t1 - t0)
	return [lo, hi, clamp(a, 0.0, 1.0)]

func sample_value(anim: Animation, track: int, t: float):
	var typ := anim.track_get_type(track)
	if typ == Animation.TYPE_POSITION_3D: return anim.position_track_interpolate(track, t)
	if typ == Animation.TYPE_ROTATION_3D: return anim.rotation_track_interpolate(track, t)
	if typ == Animation.TYPE_SCALE_3D: return anim.scale_track_interpolate(track, t)
	var r := key_range(anim, track, t)
	return anim.track_get_key_value(track, int(r[0]))

func insert_key(anim: Animation, track: int, time: float, value, transition: float = 1.0) -> void:
	for k in range(anim.track_get_key_count(track)):
		if abs(anim.track_get_key_time(track, k) - time) < EPS:
			anim.track_set_key_value(track, k, value)
			return
	anim.track_insert_key(track, time, value, transition)

func quat_angle_deg(a: Quaternion, b: Quaternion) -> float:
	var d: float = abs(a.dot(b))
	d = clamp(d, -1.0, 1.0)
	return rad_to_deg(2.0 * acos(d))

func distributed_rotation(original: Quaternion, first: Quaternion, last: Quaternion, alpha: float) -> Quaternion:
	# Difference from original end to first pose, spread from alpha=0..1 over the whole clip.
	var correction: Quaternion = last.inverse() * first
	var partial: Quaternion = Quaternion.IDENTITY.slerp(correction, alpha).normalized()
	return (original * partial).normalized()

func _init():
	var current: Animation = ResourceLoader.load(OUT_PATH, "Animation", ResourceLoader.CACHE_MODE_IGNORE)
	if current != null:
		ResourceSaver.save(current, BACKUP_PATH)

	var src: Animation = ResourceLoader.load(SRC_PATH)
	if src == null:
		print("LOAD_FAILED ", SRC_PATH)
		quit(1)
		return

	var dur: float = CUT_END - CUT_START
	var out := Animation.new()
	out.length = dur
	out.loop_mode = Animation.LOOP_LINEAR
	out.step = src.step
	var changed := 0
	var worst_before := 0.0
	var worst_before_name := ""

	for i in range(src.get_track_count()):
		var typ := src.track_get_type(i)
		var dst := out.add_track(typ)
		out.track_set_path(dst, src.track_get_path(i))
		out.track_set_interpolation_type(dst, src.track_get_interpolation_type(i))
		out.track_set_interpolation_loop_wrap(dst, true)
		out.track_set_enabled(dst, src.track_is_enabled(i))

		var name := track_name(src, i)
		var root_offset = null
		if typ == Animation.TYPE_POSITION_3D and name == "Root":
			root_offset = sample_value(src, i, CUT_START)

		var first_val = sample_value(src, i, CUT_START)
		var last_val = sample_value(src, i, CUT_END)
		if root_offset != null:
			first_val = first_val - root_offset
			last_val = last_val - root_offset

		if typ == Animation.TYPE_ROTATION_3D:
			var gap := quat_angle_deg(first_val, last_val)
			if gap > worst_before:
				worst_before = gap
				worst_before_name = name

		# Insert start.
		insert_key(out, dst, 0.0, first_val)

		for k in range(src.track_get_key_count(i)):
			var kt := src.track_get_key_time(i, k)
			if kt <= CUT_START + EPS or kt >= CUT_END - EPS:
				continue
			var nt: float = kt - CUT_START
			var alpha: float = nt / dur
			var val = src.track_get_key_value(i, k)
			if root_offset != null:
				val = val - root_offset

			if typ == Animation.TYPE_ROTATION_3D:
				val = distributed_rotation(val, first_val, last_val, alpha)
			elif typ == Animation.TYPE_POSITION_3D and name != "Root":
				# Spread positional mismatch for non-root position tracks.
				var v: Vector3 = val
				var delta: Vector3 = first_val - last_val
				val = v + delta * alpha
			elif typ == Animation.TYPE_SCALE_3D:
				var sv: Vector3 = val
				var sdelta: Vector3 = first_val - last_val
				val = sv + sdelta * alpha

			insert_key(out, dst, nt, val, src.track_get_key_transition(i, k))
			changed += 1

		# End key: root keeps forward displacement; others close exactly to first frame.
		if typ == Animation.TYPE_POSITION_3D and name == "Root":
			insert_key(out, dst, dur, last_val)
		else:
			insert_key(out, dst, dur, first_val)

	var err := ResourceSaver.save(out, OUT_PATH)
	print("SAVE err=", err, " path=", OUT_PATH)
	print("cut=", CUT_START, "..", CUT_END, " length=", dur, " changed_inner_keys=", changed)
	print("worst_original_gap=", "%0.3f" % worst_before, "@", worst_before_name)
	print("backup=", BACKUP_PATH)
	quit(err)
